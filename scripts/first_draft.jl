...
search_results =
    DataFrames.DataFrame(CSV.File(search_results_file, delim = '\t', header = column_names))
...
UnicodePlots.scatterplot(
    search_results.fident,
    log10.(search_results.evalue),
...
function create_pdb_folder_symlink(search_results::DataFrames.DataFrame)
    ...
    for row in DataFrames.eachrow(search_results)
        ...
        if chain_code !== nothing
            ...
            chain = MIToS.PDB.read(
                original_pdb_path,
                MIToS.PDB.PDBFile,
                chain = String(chain_code),
            )
...
function list_known_conformations(search_results)
    ...
    for up in uniprots
        ...
        for row in eachrow(pdbs)
            ...
            if !(target in search_results.target) &&
               !(target_with_chain in search_results.target)
...
for cluster in unique(tmalign_results.rmsd_clusters)
    ...
    open(joinpath(cluster_folder, "sequences.fasta"), "w") do file
        ...
        for target in targets
            res = read(
                joinpath(local_pdb_folder, target),
                MIToS.PDB.PDBFile,
                group = "ATOM",
                onlyheavy = true,
                occupancyfilter = true,
    ...
    run(
        `clustalo -i $(joinpath(cluster_folder, "sequences.fasta")) -o $(joinpath(cluster_folder, "sequences.aln")) --force --output-order=input-order`,
    )
    ...
    MIToS.MSA.write(
        joinpath(cluster_folder, "sequences.a3m"),
        complete_msa,
        MIToS.MSA.FASTA,
    )
    ...
    for pdb in pdbs
        res = read(
            joinpath("/alpha/database/pdb/pdb_files", pdb),
            MIToS.PDB.PDBFile,
            model = "1",
            onlyheavy = true,
            occupancyfilter = true,
        ...
        MIToS.PDB.write(
            joinpath(cluster_folder, "templates", lowercase(pdb)),
            res,
            MIToS.PDB.PDBFile,
        )
    end
