...
function compare_model(
    ...
    for model in model_files
        ...
        rmsd_holo,
        coverage,
        _=AlphaConformers.structural_alignment(structure_model, structure_holo)
        _, _, _, rmsd_apo, coverage, _ =
            AlphaConformers.structural_alignment(structure_model, structure_apo)
...
function find_model(folder_model::String)
    ...
    for cluster in cluster_dirs
        ...
        if isdir(models_path)   # Check if the path is correct
            ...
            #println(best_models)
            pdb_files = glob(
                "sequences_unrelaxed_rank_*_alphafold2_ptm_model_*_seed_*.pdb",
                models_path,
            )
...
function parse_commandline()
    s = ArgParseSettings(
        description = "Analyse AlphaConformers output: compare predicted models to apo and holo structures.",
    )
