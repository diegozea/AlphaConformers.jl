...
@testitem "Conformer clustering container fixture run" begin
    import DataFrames

    # Reuse the clean-room container's bundled fixture + expected-output contract, so the
    # artifacts the container asserts are also covered by the normal (Docker-free) test suite.
    include(joinpath(@__DIR__, "container", "fixture.jl"))

    system = "fixture_system"
    k = 3
    data_root = write_conformer_fixture(mktempdir(), system)

    # Reference-free run: the kmeans/ and hierarchical/ artifacts exist; no deepaccnet/ tree.
    out_a = mktempdir()
    res_a = cluster_conformers(
        system;
        data_root = data_root,
        out_dir = out_a,
        kmeans_k = k,
    ...
    for p in expected_artifacts(res_a)
        @test isfile(p)
    end
    @test res_a.deepaccnet_dir === nothing

    # The fixture seeds decoy templates and a search-result folder next to the prediction
    # models; the run must cluster only the nine prediction models, never the templates.
    @test DataFrames.nrow(res_a.clustering) == 9
    @test !any(
        occursin("templates", n) || occursin("fullpdb_results", n) for
        n in res_a.clustering.Name
    )

    # Score-filter run: the deepaccnet/ surviving table is added for the default cut.
    out_b = mktempdir()
    score_table = DataFrames.DataFrame(
        "Name" => res_a.clustering.Name,
        "cb-lddt" =>
            collect(range(0.1, 0.9; length = DataFrames.nrow(res_a.clustering))),
    )
    res_b = cluster_conformers(
        system;
        data_root = data_root,
        out_dir = out_b,
        kmeans_k = k,
    ...
    for p in expected_artifacts(res_b; expect_deepaccnet = true)
        @test isfile(p)
    end

    # The bundled fixture is deterministic: the same seed yields identical KMeans labels.
    out_c = mktempdir()
    res_c = cluster_conformers(
        system;
        data_root = data_root,
        out_dir = out_c,
        kmeans_k = k,
    ...
    label_col = "KMeans_K$k"
    @test res_c.clustering[!, label_col] == res_a.clustering[!, label_col]
end
