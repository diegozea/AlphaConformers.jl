    ...
    open(file, "w") do io
        for pdb in pdb_list
            ...
            if isfile(file)
                ...
                @warn "$file does not exist, it won't be included in the comparison."
            end
        end
    end
end
...
function _usalign_command(
    ...
    if outfmt != 2
        @warn "AlphaConformers needs the tabular format; changing outfmt can cause problems."
    end
    cmd = `$usalign -mol $mol -outfmt $outfmt`
    @assert isnothing(TMcut) || (0.5 <= TMcut < 1.0)
    if TMcut !== nothing
        cmd = `$cmd -TMcut $TMcut`
    end
    if fast
...
alignments unlikely to reach a TM-score of 0.5.
...
from the tool's tabular output format (`-outfmt 2`). This function uses the following
...
Those parameters are also keyword arguments to the `usalign` function. You can use those 
keyword arguments to change this default behavior. To avoid using the TMscore cutoff, you 
