...
function foldseek_api_query(pdb_file)
    foldseek_output = read(
        `curl -X POST -F q=@$pdb_file -F 'mode=3diaa' -F 'database[]=afdb50' -F 'database[]=afdb-swissprot' -F 'database[]=afdb-proteome' -F 'database[]=cath50' -F 'database[]=mgnify_esm30' -F 'database[]=pdb100' -F 'database[]=gmgcl_id' https://search.foldseek.com/api/ticket`,
        String,
    )
    foldseek_output_json = JSON.parse(foldseek_output)
    ticket = foldseek_output_json["id"]
    if foldseek_output_json["status"] != "ERROR" &&
       foldseek_output_json["status"] != "COMPLETE"
        status = JSON.parse(
            read(
                `curl -H "Content-Type: application/json" https://search.mmseqs.com/api/ticket/$ticket`,
                String,
            ),
        )
