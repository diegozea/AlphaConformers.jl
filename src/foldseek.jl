...
function foldseek_search(
    ...
    mktempdir() do tmp_folder
        ...
        run(
            `$(Foldseek_jll.foldseek()) easy-search $pdb_file $db_path $out_file $tmp_folder --format-output $format_output`,
        )
...
"""
function read_foldseek_search_results(
    file::AbstractString;
    colonnes::Vector{String} = _M8_COL_NAMES,
)
    """
...
function run_foldseek(
    ...
    try
        mktempdir() do tmp_folder
            ...
            run(
                pipeline(
                    `$(Foldseek_jll.foldseek()) createdb $pdb_file query_db --threads $n_threads`,
                ),
            )
            ...
            run(
                pipeline(
                    `$(Foldseek_jll.foldseek()) unpackdb msa msa_output --unpack-suffix a3m --unpack-name-mode 0 --threads $n_threads`,
                ),
            )
            if isfile(msa_file)
...
function _get_seq_and_columns(msa, pos_ref)
    ...
    col = [
        col for
        (col, res) in enumerate(MIToS.MSA.getsequencemapping(msa, pos_ref)) if res != 0
    ]
    (ref, col)
end
...
function _match_columns(msa_a, msa_b, pos_ref_a, pos_ref_b)
    ...
    aln = BioAlignments.alignment(
        BioAlignments.pairalign(BioAlignments.GlobalAlignment(), ref_a, ref_b, aln_model),
    )
